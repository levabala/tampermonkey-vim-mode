...
`);
  ...
  function getCurrentVisualRow(input, visualRowsInfo) {
    ...
    if (input.tagName !== "TEXTAREA") {
      return visualRowsInfo.findIndex((r) => r.logicalLine === currentLogicalLine);
    }
    try {
      input.focus();
      ...
      const fontSize = parseFloat(computedStyle.fontSize);
      const effectiveLineHeight = isNaN(lineHeight) ? fontSize * 1.2 : lineHeight;
      const relativeY = caretRect.top - inputRect.top - paddingTop - borderTop + input.scrollTop;
      const visualRow = Math.max(0, Math.floor(relativeY / effectiveLineHeight));
      return Math.min(visualRow, visualRowsInfo.length - 1);
    } catch {
      const firstRowIndex = visualRowsInfo.findIndex((r) => r.logicalLine === currentLogicalLine && r.visualRow === 1);
